...
export function safeRoi(revenue: number, timeTaken: number): number | null {
  if (typeof revenue !== 'number' || typeof timeTaken !== 'number' || timeTaken <= 0) return null;
  const roi = revenue / timeTaken;
  if (!isFinite(roi) || isNaN(roi)) return null;
